...
class TAAssignmentController:
    ...
    def assign_tas(self):
        ...
        #Skipping this updating assignment
        #self._update_assignments()
    ...
    def _update_assignments(self):
        ...
        # For each course-scholar match, update the course with the assigned TA
        for course_id, scholar_id in self.matches.items():
            course = Course.objects.get(id=course_id)
            scholar = ResearchScholar.objects.get(id=scholar_id)
            
            # Update the course with the assigned TA
            course.ta = scholar
            course.save()
            
            assigned_count += 1
            logger.info(f"Assigned {scholar.name} as TA for course {course.course_code}")
        
        logger.info(f"Updated database with {assigned_count} TA assignments")
    
    from bson import ObjectId  # Needed to convert string IDs to ObjectId
    ...
    def get_assignment_details(self):
        ...
        for course_id, scholar_id in self.matches.items():
            try:
                ...
                scholar = ResearchScholar.objects(id=ObjectId(scholar_id)).first()

                if course and scholar:
                    assignments.append({
                        'course_code': course.course_code,
                        'course_name': course.name,
                        'scholar_id': scholar.scholar_id,
                    ...
                    # You can log or skip if any is not found
                    print(f"Missing course or scholar for IDs: {course_id}, {scholar_id}")
            except Exception as e:
                print(f"Error retrieving course or scholar: {e}")
...
if __name__ == "__main__":
    ...
    # # Verify database updates
    # print("\nVerifying database updates...")
    # courses_with_tas = Course.objects(ta__ne=None)
    # print(f"Found {courses_with_tas.count()} courses with TAs assigned.")

    # for course in courses_with_tas:
    #     print(f"- Course {course.course_code}: TA = {course.ta.name}")